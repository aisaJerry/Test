(function() {
    var util = {
        ...
        getParamsUrl:function(){
            var hashDeatail = location.hash.split("?"), //location.hash  url#后面的内容   #aa?p1=1234&p2=2345
                hashName = hashDeatail[0].split("#")[1],//路由地址 '#aa'.split('#')[1] ==> aa
                params = hashDeatail[1] ? hashDeatail[1].split("&") : [],//参数内容 'p1=1234&p2=2345'.split('&') ==> [p1=1234,p2=2345]
                query = {};

            //数组转对象字面量   ==>  {p1:'1234',p2:'2345'} 
    ...
    spaRouters.prototype={
        ...
        urlChange:function(){
            ...
            if(this.routers[currentHash.path]){//currentHash.path ==> aa 如果this.routers[aa] 有值
                this.refresh(currentHash)
            }else{
                //不存在的地址重定向到首页
                location.hash = '/index' //url ==> #/index
            }
        },
        //单层路由注册
        map:function(path,callback){//对应路由名和回调函数
